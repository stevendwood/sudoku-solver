...
public class Solver {
	...
	public void solve() throws DeadEnd {
	...
	private void search() throws DeadEnd {
        ...
        for (int value: cell.getPossibleValues()) {
            ...
            try {
                ...
                this.initPossibleValues(resetPossibilities.stream().filter(x -> x.getValue() == 0).collect(Collectors.toList()));
            }
        }
        if (!this.grid.isSolved()) {
            // If we get here then we're also stuck since we haven't found a solution despite trying
            // all possible values for a cell.
            throw new DeadEnd("Tried all values for this cell  [" + cell.getRow() + ", " + cell.getCol() + "]" + cell.getPossibleValues());
        }
	}
	...
	private void initPossibleValues(List<Cell> cells) {
		cells.forEach(cell -> {
            List<Integer> peerValues = this.grid.peers(cell)
            		.stream()
        ...
        this.grid.peers(cell)
                ...
                .forEach(p -> { 
                    ...
                    if (possibleValues.size() == 0) {
                        throw new DeadEnd("No possible values for cell ["+p.getRow()+", "+p.getCol()+"] "+p.getValue());
                    }
                });
	}
	...
	private void findUniqueValuesInUnits(Cell cell) {
		if (cell != null) {
			Arrays.asList(this.grid.sameColAs(cell),
						  ...
						  this.flattenSubGrid(this.grid.sameSubGridAs(cell))).forEach(this::findUniquePossibiltyInUnit);
	...
	private void findUniquePossibiltyInUnit(List<Cell> unit) {
		unit.stream()
            ...
            List<Integer> unique = unsolvedCell.getPossibleValues()
            		.stream().filter(x -> !otherCellsPossValues.contains(x))
            		.collect(Collectors.toList());
          
            if (unique.size() == 1) {
                this.setValueForCell(unsolvedCell, unique.get(0));
            }
        });
	}
	...
	/* Unchecked exception so it can be thrown from lambda expressions */
