...
public class Solver {
	...
	private void search() throws DeadEnd {
		Cell cell = Collections.min(
				this.grid.unsolved(),
				(x, y) -> {
					int xVal = x.getPossibleValues().size() * 100
							+ (x.getRow() + x.getCol()), yVal = y
		...
		for (int value : cell.getPossibleValues()) {
			...
			try {
				...
				List<Cell> removals = this.solvedCells.subList(numSolved,
						this.solvedCells.size());
	...
	private List<Cell> flattenSubGrid(List<List<Cell>> subgrid) {
		...
		// [Cell, Cell, Cell, Cell....]
		//
		return subgrid.stream().flatMap(s -> s.stream())
				.collect(Collectors.toList());
	}
	...
	private void findUniqueValuesInUnits(Cell cell) {
		...
		if (cell != null) {
			Arrays.asList(this.grid.sameColAs(cell), this.grid.sameRowAs(cell),
		...
		unit.stream()
				...
				.forEach(
						unsolvedCell -> {
							...
							List<Integer> unique = unsolvedCell
									.getPossibleValues()
									.stream()
									.filter(x -> !otherCellsPossValues
											.contains(x))
									.collect(Collectors.toList());
